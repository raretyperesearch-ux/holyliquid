...
async function placeLpBet(wallet: string, round: Round): Promise<void> {
  ...
  // POOL BALANCING — this is the core arbitrage fix.
  // If the pool is lopsided, humans can bet the minority side for outsized
  // parimutuel payouts. LPs must actively balance by:
  //   a) ALWAYS betting the minority side (no 20% random fallback)
  //   b) Scaling bet size up proportional to the imbalance
  ...
  if (posPool === negPool) {
    // Balanced — random side, small bet
    side = Math.random() < 0.5 ? 'pos' : 'neg'
    amount = Math.min(random(BET_AMOUNTS), Math.floor(available))
  } else {
    // Lopsided — always bet minority, scale bet size with imbalance
    side = posPool < negPool ? 'pos' : 'neg'
    if (imbalance >= 10) {
      // Heavily lopsided — bet up to $5 to meaningfully balance
      amount = Math.min(5, Math.floor(available))
    } else if (imbalance >= 5) {
      // Moderately lopsided — bet $3
      amount = Math.min(3, Math.floor(available))
    } else {
      // Slightly lopsided — normal small bet
      amount = Math.min(random(BET_AMOUNTS), Math.floor(available))
    }
  }
